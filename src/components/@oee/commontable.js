...
import { dataComponentId, dataCommonTableId } from '../common';
...
function setColumnDefsAndRender(node, colDefs) {
    ...
    tables[$(node).attr(dataCommonTableId)].api.setColumnDefs(colDefs);
    Vvveb.Components.render("html/commontable@oee");
}
...
const commontable = {
    ...
    name: "Common ag-Grid",
    html: `<div ${dataComponentId}="html/commontable@oee" style="width: 500px; height: 200px;" class="dropzone draggable ag-theme-blue horizontal-stripes"></div>`,
    // onDrop(node) {
    //     $(node)
    //         .css({
    ...
    beforeInit: function (node) {
        ...
        if (!$(node).attr(dataCommonTableId)) {
            const id = index++;
            $(node).attr(dataCommonTableId, id);
            tables[id] = {
                columnDefs: [
        ...
        let i = 0;
        const properties = tables[$(node).attr(dataCommonTableId)].columnDefs.reduce((prev, cur) => {
            i++;
            prev.push({
                name: "Header " + i,
                key: "option" + i,
                ...
                onChange: function (node, value, input) {
                    ...
                    let colDefs = tables[$(node).attr(dataCommonTableId)].columnDefs;
                    if (input.nodeName == 'BUTTON') {
                        colDefs = colDefs
                            .filter((value, index) => index != keyIndex);
                        tables[$(node).attr(dataCommonTableId)].columnDefs = colDefs;
    ...
    properties: [
        {
            ...
            onChange: function (node, value) {
                ...
                const gridOptions = tables[node.attr(dataCommonTableId)];
        ...
        },
        {
            ...
            onChange: function (node) {
                const colDefs = tables[$(node).attr(dataCommonTableId)].columnDefs;
                colDefs.push({
                    headerName: 'header',
                    field: 'field',
                    width: ''
